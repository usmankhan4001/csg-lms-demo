...
> **Reconciliation note (§1.3, item 3) — the academic backbone is built, not missing.**
> `apps/api/src/db/sms_campus.py` defines `Campus` (L33), `AcademicYear` (L89),
> `AcademicTerm` (L143), `ClassSection` (L195) and `StudentEnrollment` (L298, with
> `roll_number` and `status`); the guardian custody link is `StudentGuardian` in
> `apps/api/src/db/sms_identity.py:73`; academic-year rollover is implemented in
> `apps/api/src/services/sms/academic_rollover.py`. A `Course` entity also exists
> (`apps/api/src/db/courses/courses.py:62`).
> **Still genuinely absent:** a `Program` entity, syllabus topics, and any
> Room/Classroom entity (see the §3 note). Verified against code — see
...
> **Reconciliation note (§2, box [D]) — partly stale; see the §1.3 note.** Academic
> governance masters, campuses, roll numbers, guardian custody and section enrolment all
> exist. Only `Program`, syllabus topics and the Room/Classroom entity are actually missing.
...
> **Reconciliation note (§3) — Domain 4 "Transport Fleet & Routes" is genuinely not built: correct, keep.**
> **Domain 1 "Campuses & Classrooms" is only half right:** `Campus` exists
> (`apps/api/src/db/sms_campus.py:33`), but there is **no Room/Classroom entity anywhere in
> the schema** — only free-text `room_number` columns on `ClassSection`
> (`sms_campus.py:190`), `sms_exam.py:186` and `sms_timetable.py:118`, none of which carry a
> capacity. That absence is what blocks the §6 "Academic Timetabling" constraint solver:
> `apps/api/src/services/sms/timetable_generation.py` simply copies `req.room_number`
> through (L172, L221, L237) and cannot reason about rooms it has no entity for.
> See `PROJECT_DOCS/RECONCILIATION_PLAN.md`.
...
> **Reconciliation note (§4) — 4 of these 5 handshakes are already implemented.**
> **#1 Matriculation** and **#2 Fee schedule from enrolment** are both satisfied by
> `apps/api/src/routers/sms_matriculation.py` + `apps/api/src/services/sms/matriculation.py`
> (`execute_matriculation_handshake`, L155): student and parent accounts, `SMSUserRole`
> grants (L345, L370), the `StudentGuardian` link (L392), `StudentEnrollment` (L416),
> `StudentFeeVoucher` generation (L450) and audit events (L581).
> **#4 Weighted assessment plans** is satisfied: `GradingScale` (L18), `AssessmentPlan`
> (L34), `GradebookEntry.weighted_score` (L75) and `TermReportCard` (L105) in
> `apps/api/src/db/sms_gradebook.py`, with `apps/api/src/services/sms/gradebook.py` as the
> single calculation source of truth.
> **#5 Grounded AI tutor** is satisfied: `apps/api/src/services/ai/socratic_tutor.py`
> resolves scope from `StudentEnrollment` where `status == "active"` (L147-153) and is
> Redis rate-limited (L298).
> **#3 Roster-backed attendance is the only handshake that survives as a real gap:**
> `apps/api/src/services/sms/attendance.py` contains **zero** references to
> `StudentEnrollment`, so attendance never validates that a student is actively enrolled.
> Keep this one. See `PROJECT_DOCS/RECONCILIATION_PLAN.md`.
...
> **Reconciliation note (§5) — the dynamic RBAC engine already exists; it is unwired, not unbuilt.**
> The recommendation stands, but the premise is stale: `apps/api/src/security/ems_rbac.py`
> (permission evaluation plus the `require_permission` FastAPI dependency factory, L362),
> `apps/api/src/db/ems_roles.py` and `apps/api/src/routers/ems_roles.py` are complete and
> covered by `apps/api/src/tests/security/test_ems_rbac.py`. The actual defect is that
> `require_permission` has **zero call sites in any router** — its only other occurrence in
> the source tree is the usage example inside its own docstring (`ems_rbac.py:374`). Reframe
> this as a **wiring** task, not a build task (Phase 4 of
> `PROJECT_DOCS/RECONCILIATION_PLAN.md`).
...
> **Reconciliation note (§7) — the paths in Steps 2 and 3 do not exist in this repository.**
> There is no `apps/core` (NestJS) and no `packages/database/src/schema/academics.ts`. The
> repo is `apps/api` (FastAPI + SQLModel), `apps/web` (Next.js), `apps/collab`, `apps/cli`,
> `apps/mobile`, `apps/e2e`. The real equivalents are: schema → `apps/api/src/db/` plus
> `apps/api/migrations/`; services → `apps/api/src/services/sms/`; endpoints →
> `apps/api/src/routers/`. Step 4 is likewise a wiring task rather than a build task (see the
> §5 note). For the corrected, path-accurate plan see
> `PROJECT_DOCS/RECONCILIATION_PLAN.md`.
