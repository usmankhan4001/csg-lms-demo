...
```bash
# 1. Postgres + Redis
#    .learnhouse/docker-compose.dev.yml is NOT committed -- it is generated by
#    the LearnHouse CLI's dev command (apps/cli/src/commands/dev.ts:67-73),
#    so this only works after the CLI has created it. If it is not there,
#    either run `learnhouse dev` once, or start Postgres on 5432 and Redis on
#    6379 yourself -- those are the ports run_demo_api.sh connects to
#    (learnhouse:learnhouse@localhost:5432/learnhouse, redis://localhost:6379/0).
